...
Server::Server() : _sockfd(-1), _port(0)
{
	...
	std::string one = "vini";
	std::string one3 = "vini2";

	_channels[0].addUser(one, 1);
	_channels[0].addUser(one3, 1);

	Channel tmp1("server1");
	_channels.push_back(tmp1);

	Channel tmp2("server2");
	_channels.push_back(tmp2);



		Channel tmp3("server3");
	_channels.push_back(tmp3);
...
//					std::string welcome1 = ":server CAP * LS :\r\n";
					send(user_fd, welcome.c_str(), welcome.size(), 0);
//					send(user_fd, welcome1.c_str(), welcome1.size(), 0);
				}
