...
/*   Updated: 2026/06/10 22:32:39 by vloureir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{ 
	...
	else
	{
		...
		// :vivi!vini@AN-EA7BE6BE.net.novis.pt TOPIC #a :hello you bastards
...
void Server::topic(std::vector<std::string>& av, int index)
{
	...
	for (size_t i = 0; i < channels.size(); i++)
	{
		...
		else if (av.size() == 2) // Print topic
		{
			...
			{
				std::cout << ":server 332 <nickname> <channels[i]> :" << _channels[getChannelIndex(channels[i])].getTopic() << "\r\n" << std::endl;
...
void Server::invite(std::vector<std::string>& av, int index)
{
	...
	if (av.size() == 1) // Missing arguments
	{
		std::string response = ":server 337 <nickname> :End of Invite List\r\n";
		send(_fds[index].fd, response.c_str(), response.size() + 1, 0);
	}
	else if (av.size() < 3)
	{
		std::string response = ":server 461 <nickname> INVITE :Not enough parameters\r\n";
		send(_fds[index].fd, response.c_str(), response.size() + 1, 0);
	}
	else if (!doesUserExist(av[1]))
	{
		std::string response = ":server 401 <nickname> av[1] :No such nick\r\n";
		send(_fds[index].fd, response.c_str(), response.size() + 1, 0);
	}
	else if (!isChannel(av[2]))
	{
		std::string response = ":server 403 <nickname> av[2] :No such channel\r\n";
		send(_fds[index].fd, response.c_str(), response.size() + 1, 0);		
	}
	else if (!_channels[getChannelIndex(av[2])].isOperator(_users[index].getNickname()))
	{
		std::cout << ":server 482 <nickname> <av[2]> :You're not channel operator\r\n" << std::endl;
	}
	else
	{
